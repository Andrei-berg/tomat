...
import { updateOrder, searchClients, createClient } from '@/app/actions/orders'
import type { UpdateOrderState, ClientResult } from '@/app/actions/orders'
...
function Spinner() {
  return (
    <svg className="animate-spin" width="15" height="15" viewBox="0 0 15 15" fill="none" style={{ flexShrink: 0 }}>
...
interface Props {
  ...
  initialPaymentType: 'cash' | 'card' | 'debt'
  initialClientId?: string | null
  initialClientName?: string | null
  initialDiscountPct?: number | null
  initialManualTotal?: number | null
}
...
export default function OrderEditForm({
  ...
  const [items, setItems] = useState<ItemsState>(() => {
    const base = Object.fromEntries(products.map(p => [p.id, { boxes: '', weight: '' }]))
    for (const item of initialItems) {
      base[item.product_id] = {
        boxes: item.boxes_count > 0 ? String(item.boxes_count) : '',
        weight: item.weight_kg > 0 ? String(item.weight_kg) : '',
      }
    }
    return base
  })
  const [discountPct, setDiscountPct] = useState(
    initialDiscountPct != null && initialDiscountPct > 0 ? String(initialDiscountPct) : '',
  )
  const [manualTotal, setManualTotal] = useState(
    initialManualTotal != null ? String(initialManualTotal) : '',
  )
  const [paymentType, setPaymentType] = useState<'cash' | 'card' | 'debt'>(initialPaymentType)
  ...
  const [orderState, orderAction, saving] = useActionState<UpdateOrderState, FormData>(
    updateOrder,
    undefined,
  )
  ...
  return (
    ...
    >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      style={{
                        ...
                        fontSize: '16px', fontWeight: 600, color: 'var(--mk-text)',
                        background: 'var(--mk-surface)', border: '1.5px solid var(--mk-border)',
                        borderRadius: '10px', outline: 'none', fontFamily: 'var(--font-geist-sans)', boxSizing: 'border-box',
                      }}
                    />
                  </div>
          ...
          <div style={{ flex: 1 }}>
            ...
            <input
              ...
              style={{
                ...
                fontSize: '17px', fontWeight: 600, color: 'var(--mk-text)',
                background: 'var(--mk-surface)', border: '1.5px solid var(--mk-border)',
                borderRadius: '12px', outline: 'none', fontFamily: 'var(--font-geist-sans)', boxSizing: 'border-box',
              }}
            />
            {parseFloat(discountPct) > 50 && (
              <div style={{
                marginTop: '6px', padding: '8px 12px', borderRadius: '8px',
                background: 'rgba(200, 170, 0, 0.1)', border: '1px solid rgba(200, 170, 0, 0.25)',
                color: '#c8a200', fontSize: '12px', fontWeight: 500,
              }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontSize: '14px', fontWeight: active ? 700 : 500, cursor: 'pointer',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {selectedClient ? (
          <div style={{
            ...
            padding: '0 14px', height: '44px', borderRadius: '12px',
            background: 'var(--mk-card-fill)', border: '1.5px solid var(--mk-accent)',
          }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mk-text-3)', fontSize: '18px', lineHeight: 1, padding: '4px', fontFamily: 'var(--font-geist-sans)' }}
            >
              ×
            </button>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            <input
              ...
              onFocus={() => { if (clientQuery.length >= 2 && clientResults.length > 0) setShowDropdown(true) }}
              onBlur={() => { setTimeout(() => setShowDropdown(false), 150) }}
              placeholder="Поиск клиента..."
              style={{
                display: 'block', width: '100%', height: '44px', padding: '0 12px',
                fontSize: '15px', fontWeight: 500, color: 'var(--mk-text)',
                background: 'var(--mk-surface)', border: '1.5px solid var(--mk-border)',
                borderRadius: '12px', outline: 'none', fontFamily: 'var(--font-geist-sans)', boxSizing: 'border-box',
              }}
            />
            ...
            {showDropdown && (clientResults.length > 0 || clientQuery.length >= 2) && (
              <div style={{
                ...
                background: 'var(--mk-card)', border: '1px solid var(--mk-border)', borderRadius: '12px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div style={{
          padding: '16px', borderRadius: '14px', background: 'var(--mk-card)',
          border: '1px solid var(--mk-border)', display: 'flex', flexDirection: 'column', gap: '4px',
        }}>
          <span style={{ fontSize: '12px', color: 'var(--mk-text-3)', fontWeight: 500 }}>Итого</span>
          <span style={{ fontSize: '32px', fontWeight: 700, letterSpacing: '-0.03em', color: 'var(--mk-text)', lineHeight: 1.1 }}>
            {formatRub(effective)}
          </span>
        ...
        {orderState?.error && (
          <div style={{
            ...
            color: 'var(--mk-err-text)', fontSize: '14px', fontWeight: 500,
            animationName: 'mkUp', animationDuration: '0.2s', animationFillMode: 'both',
          }}>
            {orderState.error}
          </div>
        )}
        ...
        <button
          ...
          disabled={!canSave}
          style={{
            width: '100%', height: '60px', borderRadius: '16px', marginBottom: '4px',
            background: canSave
        ...
        >
          {saving || isPending ? <><Spinner /> Сохраняю...</> : 'Сохранить изменения'}
        </button>
        ...
        <Link
          ...
          style={{
            ...
            background: 'transparent', border: '1.5px solid var(--mk-border)',
            color: 'var(--mk-text-2)', fontSize: '15px', fontWeight: 500,
